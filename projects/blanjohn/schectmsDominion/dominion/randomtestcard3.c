...
{	
	...
	for(i = 0; i < 5000; i++)
	{
		...
		testG.deck[nextPlayer][0] = c[randomCard1];
		testG.deck[nextPlayer][1] = c[randomCard2];
		tributeCardFound = 0;
		...
		for(j = 0; j < 2; j++)
		{
			if (testG.deck[nextPlayer][j] == copper || testG.deck[nextPlayer][j] == silver || testG.deck[nextPlayer][j] == gold)
        	{ 
        		treasureCardCount++;
        	}

        	else if (testG.deck[nextPlayer][j] == estate || testG.deck[nextPlayer][j] == duchy || testG.deck[nextPlayer][j] == province || testG.deck[nextPlayer][j] == gardens || testG.deck[nextPlayer][j] == great_hall)
        	{ 
        		victoryCardCount++;
        	}
        	else
