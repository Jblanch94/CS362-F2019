...
//helper function to add coins to the player
int addNumCoins(struct gameState* state, int num)
{
    return state->coins += num;
}

//refactored function used for baron
int baronRefactored(int choice1, struct gameState *state, int currentPlayer)
{
    //bug so that the number of buys for the buy phase is not increased
...
//refactored function for minion
int minionRefactored(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
    //added locally for iterators used in the function
    int i, j;

    //+1 action
    //bug never increases for the player to use another action card
    //state->numActions++;
    ...
    if (choice1)
    {
        state->coins = addNumCoins(state, 2);
    }
    else if (choice2) //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        ...
        //draw 4
        //bug introduced to draw five cards
        for (i = 0; i <= 4; i++)
        {
...
//refactored function for ambassador
int ambassadorRefactored(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
    int i;
    ...
    for (i = 0; i < state->numPlayers; i++)
    {
        //bug introduced so that no one but the person who played the card get a copy
        if (i == currentPlayer)
        {
...
//refactored function for mine
int mineRefactored(int choice1, int choice2, struct gameState* state, int currentPlayer, int handPos)
{
    int i, j;

    j = state->hand[currentPlayer][choice1]; //store card we will trash

    //bug introduced so that a choice that is not between copper and gold will be a vaild choice
    ...
    //discard card from hand
    //bug to never discard hand so the player still keeps the card
    //discardCard(handPos, currentPlayer, state, 0);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        mineRefactored(choice1, choice2, state, currentPlayer, handPos);
    case remodel:
        j = state->hand[currentPlayer][choice1]; //store card we will trash
    ...
    case baron:
        baronRefactored(choice1, state, currentPlayer);
    case great_hall:
        //+1 Card
        drawCard(currentPlayer, state);
    ...
    case minion:
        minionRefactored(choice1, choice2, state, handPos, currentPlayer);
    case steward:
        if (choice1 == 1)
        {
            //+2 cards
    ...
    case tribute:
        tributeRefactored(state, currentPlayer, nextPlayer, tributeRevealedCards);
    case ambassador:
        ambassadorRefactored(choice1, choice2, state, handPos, currentPlayer);
    case cutpurse:
